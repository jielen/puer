...
public class QueuePanel extends JPanel implements ActionListener {
  private static Logger logger=Logger.getLogger(QueuePanel.class);
  
	private JTable queueTable = new JTable(); // 显示任务队列的表格组件
	private JScrollPane scrollPane = new JScrollPane();
	private FTPClientPanel frame; // 主窗体的引用对象
	private String[] columns  = new String[] { "任务名称", "大小","方向", "状态" }; 
	 ...
	 *            主窗体
	 */
	public QueuePanel(FTPClientPanel frame) {
		this.frame = frame;
		// 从主窗体获取本地面板的上传队列
		...
		queueTimer = new Timer(1000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (localQueue.size() + ftpQueue.size() == queueTable.getRowCount()) // 如果队列大小没有改变
					return; // 结束本方法，不做任何操作
...
//		add(controlTool, EAST);
	...
	public void startQueue(boolean start) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("del")) {// 处理删除按钮的事件
			...
			if (valueAt instanceof File) {
				...
				for (int i = 0; i < size; i++) { // 遍历上传队列
					// 获取上传队列中的文件对象
					MyFtpFile ftpFile = (MyFtpFile) ftpQueue.get(i)[0];
	...
	private void up_Down_Action(final String command) {
		...
		if (orientation.equals("上传")) { // 如果是上传任务
			...
			for (int i = 0; i < size; i++) { // 遍历下载队列
				...
				MyFtpFile file = (MyFtpFile) que[0]; // 获取每个下载任务的FTP文件对象
				if (file.getAbsolutePath().equals(value)) {// 
					ListSelectionModel selModel = queueTable
...
/*		if (frame.getShutdownButton()!=null && frame.getShutdownButton().isSelected() && localQueue.isEmpty()
				&& ftpQueue.isEmpty()) {
			try {
				// 执行系统关机命令，延迟30秒钟
		...
		for (int i = 0; i < localQueueArray.length; i++) {
			...
			MyFtpFile mFile=(MyFtpFile)queueValue[1];
			
			// 把上传队列的任务添加到表格组件的数据模型中
			model.addRow(new Object[] { localFile.getAbsoluteFile(), mFile.getSize(), "上传", i == 0 ? "正在上传" : "等待上传" });
		}
		// 获取下载队列的任务
		...
		for (int i = 0; i < ftpQueueArray.length; i++) {
			...
			if (queueValue == null)
				continue;
			MyFtpFile ftpFile = (MyFtpFile) queueValue[0];
			// 把下载队列的任务添加到表格组件的数据模型中
			model.addRow(new Object[] { ftpFile, ftpFile.getSize(), "下载", i == 0 ? "正在下载" : "等待下载" });
		}
