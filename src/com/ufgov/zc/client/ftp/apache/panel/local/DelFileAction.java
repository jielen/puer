  ...
  private static Logger logger=Logger.getLogger(DelFileAction.class);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (selRows.length < 1) // 如果没有选择表格内容
			return; // 结束该方法
		int confirmDialog = JOptionPane.showConfirmDialog(localPanel, "确定要删除选中文件吗？"); // 用户确认是否删除
      ...
      for (int i = 0; i < selRows.length; i++) {
        // 获取每个选择行的第一列单元内容
        Object value = DelFileAction.this.localPanel.localDiskTable.getValueAt(selRows[i], 0);
        ...
        DelFileAction.this.localPanel.refreshFolder(parentFile);
				...
				private void delFile(File file) {
					try {
						if (file.isFile()) { // 如果删除的是文件
							...
							if (!delete) {
								JOptionPane.showMessageDialog(localPanel, file.getAbsoluteFile()
										+ "文件无法删除。", "删除文件",JOptionPane.ERROR_MESSAGE);
								return;
							}
							...
							if (!delete) { // 如果成功删除
								JOptionPane.showMessageDialog(localPanel, file.getAbsoluteFile()
										+ "文件夹无法删除。", "删除文件",JOptionPane.ERROR_MESSAGE);
								return; // 返回方法的调用处
							}
						}
					} catch (Exception ex) {
