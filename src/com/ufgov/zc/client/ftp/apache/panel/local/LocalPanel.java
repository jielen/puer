...
import com.ufgov.zc.client.ftp.apache.panel.ftp.TableConverter;
...
public class LocalPanel extends javax.swing.JPanel {
  ...
  public LocalPanel(FTPClientPanel clientPanel) {
    frame = clientPanel;
  ...
  private void _initLocalF() {
	  if(localDiskComboBox.getModel().getSize()>0){
		  localDiskComboBox.setSelectedIndex(0);
		  File file=(File) localDiskComboBox.getSelectedItem();
		  refreshFolder(file);
	  }
}
...
/**
  ...
  private void initComponents() {
    ...
    /**
     * 向现有边框添加一个标题，使其具有指定的位置和默认字体和文本颜色（由当前外观确定）。 TitledBorder.CENTER:
     * 将标题文本置于边框线的中心。 TitledBorder.ABOVE_TOP: 将标题置于边框顶端线的上部。
     */
    setBorder(javax.swing.BorderFactory.createTitledBorder(null, "本地", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.ABOVE_TOP));
      ...
      if (evt.getClickCount() >= 2) { //  如果是双击鼠标
        if (selFile.isDirectory()) { // 并且选择的是文件夹
          ...
          try {
            ...
            Logger.getLogger(FTPClientPanel.class.getName()).log(Level.SEVERE, null, ex);
          }
        }
      }
   ...
   * 读取本地文件到表格的方法
   */
  private void listLocalFiles(File curDir) {
    if (curDir == null || curDir.isFile()) { return; }
    localSelFilePathLabel.setText(curDir.getAbsolutePath());
    File[] listFiles = curDir.listFiles(); // 获取磁盘文件列表
    // 获取表格的数据模型
    ...
    model.addRow(new Object[] { "..", "<DIR>", "" }); // 创建..选项
    if (listFiles == null) {
      JOptionPane.showMessageDialog(this, curDir.getName() + "该磁盘无法访问!");
      return;
    }
    ...
    for (int i = 0; i < listFiles.length; i++) {
      File diskFile = new DiskFile(listFiles[i]); // 创建文件对象
      String sizeString = "";
      if (listFiles[i].isDirectory()) {
        sizeString = "<DIR>";
      } else {
        sizeString = fu.parseLength(listFiles[i].length());
      }
      ...
      // 将单个文件的信息添加到表格的数据模型中
      model.addRow(new Object[] { diskFile, sizeString, fu.parseDate(calendar.getTime()) });
    }
  ...
  public void stopUploadThread() {
    if (uploadThread != null) uploadThread.stopThread();
  }
  ...
  public void enableUpload(boolean enableUpload){
