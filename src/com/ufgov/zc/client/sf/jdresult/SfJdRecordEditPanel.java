...
public class SfJdRecordEditPanel extends AbstractMainSubEditPanel {
	  ...
	  TextAreaFieldEditor jdResult = new TextAreaFieldEditor(LangTransMeta.translate(SfJdResult.COL_JD_RESULT), "jdResult", -1, 5, 5);
	  ...
	  TextAreaFieldEditor jdRequire = new TextAreaFieldEditor(LangTransMeta.translate(SfEntrust.COL_JD_REQUIRE), "entrust.jdRequire", -1, 9, 5);
	    
	  private JTabbedPane baseInfoPanel;
	  ...
	  String titleJdResult = "分析说明";

	  String titleZhusu = "主诉";

	  String titleJdTarget = "鉴定对象";
	  ...
	  private void addSubPane() {
		    ...
		    parent.setSize(parent.getSize().width + 100, parent.getSize().height + 100);
...
//		        logger.debug("=========+----//下面一句是为了打开word后刷新窗口--================opened word "+isSuccess);
		        if (isSuccess) {

		          //下面一句是为了打开word后刷新窗口

		          parent.setSize(parent.getSize().width - 10, parent.getSize().height - 10);
...
//		          logger.debug("&&&&&&&&&---------------&&&&&&&&&&&&&&&&&&=下面一句是为了打开word后刷新窗口");
		    ...
		    tabPane.addTab("检验记录", wordPane);
//		    tabPane.addTab(wordPane,"检验记录", 0);
//		    tabPane.add(wordPane,"检验记录", 0);
...
private String refreshMainData(){
    ...
    return bill.getFileId();
	  ...
	  private void refreshSubData(String fileID) {
	  ...
	  private synchronized void closeWordPanel(WordPane wp, boolean isSave) {
	  ...
	  protected void doPrintReport(boolean printZb) {
	    ...
	    if (SfJdResult.RESULT_TYPE_JYBG.equalsIgnoreCase(jdResult.getResultType())) {
	      ...
	      userData.put(IWordHandler.FILE_NAME, jdResult.getEntrust().getCode() + "检验意见书" + zfb);
	  ...
	  private void doPrintJdRecord() {
	    ...
	    userData.put(IWordHandler.FILE_NAME, entrust.getEntrustCode() + "鉴定记录");
...
/*
	    ...
	    basePanel.add(baseInfoPanel, BorderLayout.CENTER);
	    workPanel.add(footPanel, BorderLayout.SOUTH);
	     
	    tabPane.add("案情信息",basePanel);

//	    tabPane.add(basePanel,"案情信息",0);
	  ...
	  private void initBaseInfoPanel() {
	    ...
	    initMaterialsTablePanel();
	    
	    baseInfoPanel.add(titleJdTarget, jdTargetDetailPanel);

	    baseInfoPanel.add("检案摘要", brief);

	    baseInfoPanel.addTab("送检材料", materialsTablePanel);

	    baseInfoPanel.add("鉴定要求", jdRequire);

	    //    centerPanel.add(titleZhusu, zhuSu);

//	    centerPanel.add("检验过程", jdProcess);

//	    centerPanel.add(titleJdResult, jdResult);

//	    centerPanel.add("鉴定意见", jdOpinion);

	    baseInfoPanel.add("备注", remark);
	  }
	  public JComponent initMaterialsTablePanel() {
...
/*
	  ...
	  private void adjustTabs() {
	    ...
	    if (bill.getEntrust().getMajorCode().equals(fayiCode)) {
	      ...
	      /*if (!hideTabs.contains(titleJdResult)) {
	        centerPanel.remove(jdResult);
	        hideTabs.add(titleJdResult);
	      }*/
	    } else {
	      /* if (!hideTabs.contains(titleZhusu)) {
	      ...
	      if (!hideTabs.contains(titleJdTarget)) {
	        baseInfoPanel.remove(jdTargetDetailPanel);
	        hideTabs.add(titleJdTarget);
	      }
	      /*if (hideTabs.contains(titleJdResult)) {
	        centerPanel.add(titleJdResult, jdResult);
	        hideTabs.remove(titleJdResult);
	      }*/
	    }
	  }
	}
