 ...
 *
 */
public class ZcFaCardListPanel extends AbstractEditListBill implements ParentWindowAware {

  public static final Logger logger = Logger.getLogger(ZcFaCardListPanel.class);

  private ZcFaCardListPanel self = this;

  private Window parentWindow;

  public static final String compoId = "SF_FA_CARD";
...
public ZcFaCardListPanel(){
  ...
  UIUtilities.asyncInvoke(new DefaultInvokeHandler<List<SearchCondition>>() {
    ...
    public List<SearchCondition> execute() throws Exception {
      ...
      .getCurrUserId(), ZcFaCard.TAB_ID_ZC_FA_CARD);
...
private AbstractDataDisplay createDataDisplay(List<TableDisplay> showingDisplays) {
  ...
  return new DataDisplay(SearchConditionUtil.getAllTableDisplay(ZcFaCard.TAB_ID_ZC_FA_CARD), showingDisplays,
...
private final class DataDisplay extends MultiDataDisplay {
  ...
  boolean showConditionArea) {
    ...
    super(displays, showingDisplays, conditionArea, showConditionArea, ZcFaCard.TAB_ID_ZC_FA_CARD);
      ...
      table.addMouseListener(new MouseAdapter() {
        ...
        public void mouseClicked(MouseEvent e) {
          ...
          if (e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
            ...
            new ZcFaCardDialog(self, viewList, row);
  ...
  protected void handleTableDisplayActived(AbstractSearchConditionItem[] searchConditionItems, final TableDisplay tableDisplay) {
    ...
    UIUtilities.asyncInvoke(new DefaultInvokeHandler<TableModel>() {
      ...
      public TableModel execute() throws Exception {
        ...
        return ZcFaCardToTableModelConverter.convertToTableModel(self.zcFaCardServiceDelegate.getCardLst(elementConditionDto, requestMeta));
...
static {
  ...
  LangTransMeta.init("FA%");
  LangTransMeta.init("CARD%");
  ...
  public static void main(String[] args) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
      ...
      public void run() {
        ...
        ZcFaCardListPanel bill = new ZcFaCardListPanel();
  ...
  public void doUnAudit() {
    ...
    try {
      ...
      for (int i = 0; i < beanList.size(); i++) {
        ...
        ZcFaCard make = (ZcFaCard) beanList.get(i);

        make.setAuditorId(WorkEnv.getInstance().getCurrUserId());

        zcFaCardServiceDelegate.unAuditFN(make, requestMeta);
  ...
  public void doSuggestPass() {
    ...
    try {
      ...
      for (int i = 0; i < beanList.size(); i++) {
        ...
        card.setIsGoonAudit(ZcSettingConstants.IS_GOON_AUDIT_NO);

        zcFaCardServiceDelegate.auditFN(card, requestMeta);
  ...
  private void executeAudit(List beanList, int isGoonAudit) {
    ...
    try {
      ...
      for (int i = 0; i < beanList.size(); i++) {
        ...
        card.setIsGoonAudit(isGoonAudit);

        card.setComment(comment);

        card.setAuditorId(WorkEnv.getInstance().getCurrUserId());

        zcFaCardServiceDelegate.updateFN(card, requestMeta);

      }

      zcFaCardServiceDelegate.commitListFN(beanList, requestMeta);
...
//    topDataDisplay.getActiveTableDisplay().getTable().setModel(ZcEbNoticeToTableModelConverter.convertToTableModel(beanList));
  ...
  public List getCheckedList() {
    ...
    List<ZcFaCard> beanList = new ArrayList<ZcFaCard>();
    ...
    for (Integer checkedRow : checkedRows) {
      ...
      ZcFaCard bean = (ZcFaCard) list.get(accordDataRow);
  ...
  private void doAdd() {
    ...
    new ZcFaCardDeprStyleDialog(self);
  ...
  private void doDelete() {
    ...
    try {
      ...
      for (Iterator iterator = beanList.iterator(); iterator.hasNext();) {
        ...
        card = (ZcFaCard) iterator.next();

        if (!"0".equals(card.getCardStatu())){

          JOptionPane.showMessageDialog(this, "非编辑状态单据，不可以删除！", "提示", JOptionPane.ERROR_MESSAGE);
          return;
        }

        zcFaCardServiceDelegate.deleteListFN(beanList, this.requestMeta);
  ...
  private void doTrace() {
    ...
    for (int i = 0; i < beanList.size(); i++) {
      ...
      ZcFaCard bean = (ZcFaCard) beanList.get(i);
  ...
  private void doTraceDataButton() {
    ...
    ZcFaCard sh = (ZcFaCard) beanList.get(0);

//    DataFlowConsoleCanvas dfc = new DataFlowConsoleCanvas(sh.getCardId(), this.compoId);
//
//    dfc.showWindow();
