...
public class ZebraPrinter {
  ...
  public static void main(String[] args) {
    ZebraPrinter p = new ZebraPrinter("ZDesigner GK888t (EPL)");
    //1.打印单个条码
    String bar0 = "1234567890";//条码内容
//    String bar0Zpl = "^FO110,110^BY6,3.0,280^BCN,,Y,N,N^FD${data}^FS";//条码样式模板
    String bar0Zpl = "^FO20,20^BY5,2.0,60^BCN,,Y,N,N^FD${data}^FS";//条码样式模板
...
//    boolean result1 = p.print(zpl);//打印
    ...
    //左边的条码
    String bar1 = "07";
//    p.setChar(bar1, 10, 30, 20, 20);
//    zpl = p.getZpl();
    System.out.println(zpl);
//    p.print(zpl);//打印
    
    String bar1Zpl = "^FO10,10^BY3,3.0,30^BCR,,N,N,N^FD${data}^FS";//条码样式模板
...
//    String bar2 = "00000999990018822969";//20位
    String bar2 = "JC20172969";//10位
    String bar2Paper = "^FO5,160^BY2,1.0,60^BCN,,Y,N,N^FD${data}^FS";//条码样式模板
    p.setBarcode(bar2,bar2Paper);
    p.setText("普司鉴委托[2016]第1197号", 5, 80, 20, 40, 15, 1, 1, 20);
    zpl = p.getZpl();
    System.out.println(zpl);
    p.print(zpl);//打印
...
//    boolean result2 = p.print(zpl2);
   ...
   */
  public ZebraPrinter(String printerURI) throws SfBusinessException{
    this.printerURI = printerURI;
    //加载字体
    File file=getFontLibFile();
    if(file==null){
      System.out.println("字库文件ts24.lib不存在");
      throw new SfBusinessException("字库文件ts24.lib不存在,无法打印中文."); 
    }
    if(file.exists()){
      FileInputStream fis;
      try {
    ...
    if (printService == null) {
      ...
      throw new SfBusinessException("打印出错：没有找到打印机：["+printerURI+"]"); 
      //循环出所有的打印机
     /* if (services != null && services.length > 0) {
   ...
   * 读取字体文件ts24.lib
   * 默认两个位置：
   * 一个是C:\Documents and Settings\Administrator\AppData\Roaming\platform\localResource,这里是applet资源初始化时下载到这里的
   * 另一个是C:\根目录
   * @return
   */
  private File getFontLibFile(){
    String dir=System.getenv("APPDATA") + "/platform/localResource/";
    File file=new File(dir+"ts24.lib");
    if(file.exists()){
      return file;
    }
    file=new File("C:\ts24.lib");
    if(file.exists()){
      return file;
    } 
    return null;
   ...
   */
  public void setText(String str, int x, int y, int eh, int ew, int es, int mx, int my, int ms) throws SfBusinessException {
    /*if(dotFont==null){
      throw new SfBusinessException("字库文件ts24.lib不存在,无法打印中文.");
    }*/
   ...
   * @param zpl 完整的ZPL
   */
  public boolean print(String zpl) throws SfBusinessException{
    /*if(printService==null){
    ...
    try {
      ...
      e.printStackTrace();
      throw new SfBusinessException("打印出错："+e.getMessage(),e);  
    }
  }
  /**
   * 字符串转byte[]
