...
import com.ufgov.zc.client.common.converter.sf.SfDossierToTableModelConverter;
 ...
 *
 */
public class SfDossierListPanel  extends AbstractEditListBill implements ParentWindowAware {
  ...
  public static final Logger logger = Logger.getLogger(SfDossierListPanel.class);

  private SfDossierListPanel self = this;

  private Window parentWindow;

  public static final String compoId = "SF_DOSSIER";
  ...
  public SfDossierListPanel(SfEntrust entrust){
    this.entrust=entrust;
    init();
  }
public SfDossierListPanel(){
  init();
}
 void init(){
   ...
   UIUtilities.asyncInvoke(new DefaultInvokeHandler<List<SearchCondition>>() {
     ...
     public List<SearchCondition> execute() throws Exception {
       ...
       .getCurrUserId(), SfDossier.TAB_ID);
...
private AbstractDataDisplay createDataDisplay(List<TableDisplay> showingDisplays) {
  ...
  return new DataDisplay(SearchConditionUtil.getAllTableDisplay(SfDossier.TAB_ID), showingDisplays,
...
private AbstractSearchConditionArea createTopConditionArea() {
  ...
  topSearchConditionArea = new SaveableSearchConditionArea(SfDossier.SEARCH_ID, null, true, defaultValueMap, null);
...
private final class DataDisplay extends MultiDataDisplay {
  ...
  boolean showConditionArea) {
    ...
    super(displays, showingDisplays, conditionArea, showConditionArea, SfDossier.TAB_ID);
      ...
      table.addMouseListener(new MouseAdapter() {
        ...
        public void mouseClicked(MouseEvent e) {
          ...
          if (e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
            ...
            SfDossier charge=(SfDossier) viewList.get(row);
            if(self.getParent() instanceof JClosableTabbedPane){
              new SfDossierDialog(self, viewList, row, topDataDisplay.getActiveTableDisplay().getStatus());
            }else{
  ...
  protected void handleTableDisplayActived(AbstractSearchConditionItem[] searchConditionItems, final TableDisplay tableDisplay) {
    ...
    UIUtilities.asyncInvoke(new DefaultInvokeHandler<TableModel>() {
      ...
      public TableModel execute() throws Exception {
        return SfDossierToTableModelConverter.convertMainLst(self.sfDossierServiceDelegate.getMainDataLst(elementConditionDto, requestMeta));
  ...
  public static void main(String[] args) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
      ...
      public void run() {
        ...
        SfDossierListPanel bill = new SfDossierListPanel();
...
//    toolBar.add(traceDataButton);
  ...
  public void refreshCurrentTabData(List beanList) {
    ...
    topDataDisplay.getActiveTableDisplay().getTable().setModel(SfDossierToTableModelConverter.convertMainLst(beanList));

  }

  public List getCheckedList() {

    List<SfDossier> beanList = new ArrayList<SfDossier>();
    ...
    for (Integer checkedRow : checkedRows) {
      ...
      SfDossier bean = (SfDossier) list.get(accordDataRow);
  ...
  private void doAdd() {
    ...
    new SfDossierDialog(self, new ArrayList(1), -1, topDataDisplay.getActiveTableDisplay().getStatus());
...
//    new SfDataFlowDialog(compoId,null,self);
  ...
  private void doTrace() {
    ...
    for (int i = 0; i < beanList.size(); i++) {
      ...
      SfDossier bean = (SfDossier) beanList.get(i);
