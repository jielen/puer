 ...
 */
package com.ufgov.zc.client.jftp;
...
import com.ufgov.zc.client.jftp.gui.base.AppMenuBar;
import com.ufgov.zc.client.jftp.gui.base.DownloadList;
import com.ufgov.zc.client.jftp.gui.base.DownloadQueue;
import com.ufgov.zc.client.jftp.gui.base.LocalDir;
import com.ufgov.zc.client.jftp.gui.base.LogFlusher;
import com.ufgov.zc.client.jftp.gui.base.RemoteDir;
import com.ufgov.zc.client.jftp.gui.base.StatusPanel;
import com.ufgov.zc.client.jftp.gui.base.dir.Dir;
import com.ufgov.zc.client.jftp.gui.base.dir.DirEntry;
import com.ufgov.zc.client.jftp.gui.framework.FileTransferable;
import com.ufgov.zc.client.jftp.gui.framework.GUIDefaults;
import com.ufgov.zc.client.jftp.gui.framework.HDesktopBackground;
import com.ufgov.zc.client.jftp.gui.framework.HImage;
import com.ufgov.zc.client.jftp.gui.hostchooser.HostChooser;
import com.ufgov.zc.client.jftp.gui.hostchooser.SftpHostChooser;
import com.ufgov.zc.client.jftp.gui.tasks.HostInfo;
import com.ufgov.zc.client.jftp.net.BasicConnection;
import com.ufgov.zc.client.jftp.net.ConnectionHandler;
import com.ufgov.zc.client.jftp.net.ConnectionListener;
...
import com.ufgov.zc.client.jftp.system.LocalIO;
import com.ufgov.zc.client.jftp.system.UpdateDaemon;
...
public class JFtp extends JPanel implements WindowListener, ComponentListener,
	...
	public static JLabel statusL = new JLabel(
			"Welcome to JFtp...                                                            ");
	 ...
	 * 准备无远程服务时的信息和面板
	 */
	private JPanel initNoFtpMsg(String msg) {
		// 远程文件服务未联通！
		// 远程文件服务连接中。。。。。。
		if (noFtpLabel == null) {
			noFtpLabel = new JLabel();
			noFtpLabel.setFont(new java.awt.Font("Dialog", 1, 15));
			noFtpLabel.setForeground(Color.blue);
		}
		noFtpLabel.setText(msg);
		noFtpPanel.setBorder(new EmptyBorder(50, 10, 10, 10));
		noFtpPanel.setLayout(new BorderLayout());
		noFtpPanel.add(noFtpLabel, BorderLayout.NORTH);
		return noFtpPanel;
	}

	public void ftpConnectionFailed() {
		// remoteConnectionPanel.remove(noFtpPanel);
		remoteConnectionPanel.removeAll();
		remoteConnectionPanel.addTab("远程文件:", null,
				initNoFtpMsg("远程文件服务无法联通！"), "远程文件");
		remoteConnectionPanel.setSelectedIndex(0);
		remoteConnectionPanel.validate();

	}

	public void ftpConnectionSuccesed() {
		if (remoteConnectionPanel.getTabCount() > 0) {
			Vector vs = new Vector();
			for (int i = 0; i < remoteConnectionPanel.getTabCount(); i++) {
				if (remoteConnectionPanel.getComponentAt(i) instanceof Dir) {
					continue;
				} else {
					vs.add(remoteConnectionPanel.getComponentAt(i));
				}
			}
			if (vs.size() > 0) {
				for (int i = 0; i < vs.size(); i++) {
					remoteConnectionPanel.remove((Component) vs.get(i));
				}
			}
		}
		if (remoteConnectionPanel.getTabCount() > 0) {
	...
	public void init() {
		...
		j1 = new JInternalFrame("本地文件", true, false, true, true);
		j1.setMinimumSize(new Dimension(300, 300));
		j1.setLocation(5, 5);
		localConnectionPanel
				.addTab("本地文件:", null, (Component) localDir, "本地文件");
		...
		j2 = new JInternalFrame("服务器文件", true, false, true, true);
		j2.setLocation(430, 5);
		// remoteConnectionPanel.addTab("远程文件://", null, (Component)
		// remoteDir,"远程文件");

		remoteConnectionPanel.addTab("远程文件:", null,
				initNoFtpMsg("远程文件服务连接中。。。。。。"), "远程文件");
		remoteConnectionPanel.setSelectedIndex(0);
		...
		/*
		 * j5 = new JInternalFrame("批量下载队列", true, false, true, true);
		 * j5.setLocation(5, 400); j5.getContentPane().add(dQueue,
		 * BorderLayout.CENTER); desktop.add(j5); j5.pack(); j5.setSize(new
		 * Dimension(440, 150)); j5.show();
		 */

		j3 = new JInternalFrame("日 志", true, false, true, true);
		...
		j4 = new JInternalFrame("上传下载情况", true, false, true, true);
		...
		/*
		 * if(!mainUsed) { SwingUtilities.invokeLater(new Runnable() { public
		 * void run() { chooseHost(); } }); } else { chooseHost(); }
		 */
	...
	public void windowClosing(WindowEvent e) {
		...
		/*
		 * if(!mainUsed) { Settings.setProperty("jftp.window.x", (int)
		 * this.getLocationOnScreen().getX());
		 * Settings.setProperty("jftp.window.y", (int)
		 * this.getLocationOnScreen().getY()); } else {
		 * Settings.setProperty("jftp.window.x", (int)
		 * mainFrame.getLocationOnScreen().getX());
		 * Settings.setProperty("jftp.window.y", (int)
		 * mainFrame.getLocationOnScreen().getY()); }
		 */
		...
		/*
		 * if(Settings.isStandalone) { System.exit(0); } else {
		 * mainFrame.dispose(); }
		 */
		System.exit(0);
	}
	...
	private static void compile() {
		/*
		 * try { Compiler.enable();
		 * System.out.println(Compiler.compileClass(Class
		 * .forName("java.util.Hashtable")));
		 * System.out.println(Compiler.compileClasses
		 * ("com.ufgov.zc.client.jftp"));
		 * System.out.println(Compiler.compileClasses
		 * ("com.ufgov.zc.client.jftp.JFtp"));
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					frame.setTitle(Settings.title + " - Version "+ getVersion());

					frame.setResizable(Settings.resize);
					frame.addWindowListener(jftp);

					Image icon = HImage.getImage(jftp, Settings.iconImage);
					frame.setIconImage(icon);
					frame.setFont(GUIDefaults.font);

					// *** MY CHANGES
					// frame.setJMenuBar(new AppMenuBar(jftp));
					// menuBar = new AppMenuBar(jftp);
					// frame.setJMenuBar(menuBar);

					// ***
					frame.getContentPane().setLayout(new BorderLayout());
					frame.getContentPane().add("Center", jftp);
					...
					jftp.startFtpConnection("127.0.0.1", 21, "jielen",
							"123456", "/record/2018/4/", 3);
				} catch (Error ex) {
					ex.printStackTrace();
				}
			}
	...
	public static void mainold(String[] argv) {
		try {
			...
			JFtp jftp = new JFtp(true);
			/*
			 * UIManager.getLookAndFeelDefaults().put("ClassLoader",jftp.getClass
			 * ().getClassLoader());
			 * 
			 * //boolean lookSet = false; String tmp =
			 * Settings.getLookAndFeel();
			 * 
			 * //UIManager.installLookAndFeel("Metouia",
			 * "net.sourceforge.mlf.metouia.MetouiaLookAndFeel"); if(tmp !=
			 * null) { jftp.setLookAndFeel(Settings.getLookAndFeel()); } else {
			 * jftp
			 * .setLookAndFeel("net.sourceforge.mlf.metouia.MetouiaLookAndFeel"
			 * ); }
			 * 
			 * //else //{ // jftp.setLookAndFeel(
			 * "com.incors.plaf.kunststoff.KunststoffLookAndFeel"); // lookSet =
			 * true; //} //if(!lookSet)
			 * UIManager.installLookAndFeel("Kunststoff",
			 * "com.incors.plaf.kunststoff.KunststoffLookAndFeel");
			 * 
			 * 
			 * Don't try to add the Kunststoff look and feel if it has already
			 * been added.
			 * 
			 * if((Settings.getLookAndFeel() == null) ||
			 * !Settings.getLookAndFeel
			 * ().equals("com.incors.plaf.kunststoff.KunststoffLookAndFeel")) {
			 * 
			 * Somehow even though UIManager.installLookAndFeel throws a
			 * ClassNotFoundException, it ends up getting added to the list of
			 * installed look and feels anyway. What we do here is make Java
			 * throw a ClassNotFoundException before then by checking whether or
			 * not the class exists, and only if the class exists does it then
			 * get added to the list of available look and feel's.
			 * 
			 * try {
			 ...
			 * 
			 * 
			 * Don't try to add the Metouia look and feel if it has already been
			 * added.
			 * 
			 * if((Settings.getLookAndFeel() == null) ||
			 * !Settings.getLookAndFeel
			 * ().equals("net.sourceforge.mlf.metouia.MetouiaLookAndFeel")) {
			 * 
			 * Somehow even though UIManager.installLookAndFeel throws a
			 * ClassNotFoundException, it ends up getting added to the list of
			 * installed look and feels anyway. What we do here is make Java
			...
			if (argv.length > 0) {
				if (argv[0].indexOf("sftp:") >= 0) {
					new SftpHostChooser().update(argv[0].replaceAll("sftp:",
							"ftp:"));
				} else {
	...
	private void log(String msg) {
		 ...
		 * repaint(); //paintImmediately(0, 0, getSize().width,
		 * getSize().height); } });
		 */
		repaint();
		revalidate();
	...
	private static void checkForUpgrade() {
		try {
			...
			if (!dl.exists() || (dl.length() <= 0)) {
				URL url = new URL(
						"http://osdn.dl.sourceforge.net/sourceforge/j-ftp/"
								+ nextVersion);
				BufferedOutputStream f = new BufferedOutputStream(
						new FileOutputStream(dl));
	...
	public void ensureLogging() {
		if (buffer.equals("")) {
			...
			if ((logSp == null)
					|| ((bar = logSp.getVerticalScrollBar()) == null)
					|| bar == null || (bar.getValue() == bar.getMaximum())
	...
	public void addConnection(String name, BasicConnection con) {
		...
		int x = remoteConnectionPanel.getSelectedIndex();// 远程文件
		// remoteConnectionPanel.addTab(parse(name), null, (Component) tmp,
		// "Switch to: " + parse(name));
		remoteConnectionPanel.addTab("远程文件", null, (Component) tmp, "远程文件");
	...
	public void stateChanged(ChangeEvent e) {
		if (!(remoteConnectionPanel.getSelectedComponent() instanceof Dir)) {
			return;
		}
	...
	public void drop() {
		try {
			handleDrop(null, Toolkit.getDefaultToolkit().getSystemClipboard()
					.getContents(this));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	 ...
	 * @param user 用户
	 * @param password 密码
	 * @param defaultPath ftp上可以访问的根目录
	 * @param maxCollection 最大连接数
	 */
	public void startFtpConnection(String host, int port, String user,
			String password, String defaultPath, int maxCollection) {
		if (host == null)
			return;
		...
		Settings.setProperty("jftp.ftpPasvMode", true);
		...
		JFtp.hostinfo.hostname = host;
		JFtp.hostinfo.username = user;
		JFtp.hostinfo.password = password;
		JFtp.hostinfo.port = "" + port;
		JFtp.hostinfo.type = "ftp";

		if (maxCollection <= 0) {
			Settings.maxConnections = 3;
		} else {
			Settings.maxConnections = maxCollection;
		}

		Settings.save();

		SaveSet s = new SaveSet(Settings.login_def, host, user, password, ""
				+ port, defaultPath, Settings.defaultWorkDir);

		int response = StartConnection.startFtpCon(host, user, password, port,
				defaultPath, false, "<default>");
	}
	public void close(){
