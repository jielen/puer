 ...
 */
package com.ufgov.zc.client.jftp.net;
...
//            if(initCWD.trim().equals(Settings.defaultDir) ||!chdirNoRefresh(initCWD)) --change by chenjl 20180404

            if(initCWD.trim().equals(Settings.defaultDir) ||!chOrMkDir(initCWD))
                   //
            {
...
//                    else if(f.exists() && Integer.parseInt(size) > 0)
    ...
    */
	...
	//String tmpPath = p;
    	...
    	if(p!=null && (p.equals(initCWD+"..")
    					||p.equals(initCWD+"../")
    					||(p.equals("..") && initCWD.equals(pwd))//点击工具栏上的往上级按钮,p是“..”
    					)){//已经是初始的根目录，不能进入到上层目录了
    		return false;    		
    	}
    ...
    /**
     * 进入当前指定的路径，如果没有的话，根据路径创建对应的文件夹
     * @param p 指定路径
     * @return
     */
    public boolean chOrMkDir(String p){
    	
    	boolean cdDir=chdirWork(p);
    	if(!cdDir){
    		if(p.endsWith("/")){
    			p=p.substring(0, p.lastIndexOf("/"));
        	}
    		String lastDir=p.substring(p.lastIndexOf("/")+1);
    		//进入上级目录
